...
public class VGAGroupGenerator {
    ...
    public static Set<VGAVehiclePlan> generateGroupsForVehicle(RideSharingOnDemandVehicle v, List<VGARequest> requests, int noOfVehicles) {
        ...
        groups.add(new VGAVehiclePlan(v, new LinkedHashSet<VGARequest>()));
        ...
        for (VGARequest r : requests) {
            ...
            VGAVehiclePlan plan;
            if((plan = optimalVehiclePlanPermutations(v, new VGAVehiclePlan(v, g))) != null) {
        ...
        while (currentGroups.size() > 0) {
            ...
            for (Set<VGARequest> g : currentGroups) {
                for (VGARequest r : feasibleRequests) {
                    ...
                    VGAVehiclePlan plan;
                    if(add && (plan = optimalVehiclePlanPermutations(v, new VGAVehiclePlan(v, toCheck))) != null) {
    ...
    public static Set<VGAVehiclePlan> generateDroppingVehiclePlans(RideSharingOnDemandVehicle v, Set<VGARequest> requests) {
        Set<VGAVehiclePlan> droppingPlans = new LinkedHashSet<>();

        droppingPlans.add(new VGAVehiclePlan(v, new LinkedHashSet<VGARequest>()));
    ...
    public static VGAVehiclePlan optimalVehiclePlanPermutations(RideSharingOnDemandVehicle v, VGAVehiclePlan vp){
            ...
            for(VGARequest r : plan.getActiveRequests()){
                ...
                if(r.getDestination().getWindow().isInWindow(simplerPlan.getCurrentTime())) {
                    ...
                    if ((simplerPlan.getCurrentTime() - r.getOriginTime()) <= MathUtils.DELTA_R_MAX *
                            MathUtils.getTravelTimeProvider().getTravelTime(VehicleGroupAssignmentSolver.getVehicle(), r.getOriginSimulationNode(), r.getDestinationSimulationNode()) + 0.001
