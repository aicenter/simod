...
# 		  normed=True)
#axes.hist([delays_1, delays_2], bins, label=['Before Superblock', 'After Superblock'], normed=True)
axes.hist([delays_1 - delays_2], bins, label=['Difference between travel times'], normed=True)
axes.yaxis.set_major_formatter(FuncFormatter(to_percent))

# fig, axes = plt.subplots(1, 1, subplot_kw={"adjustable": 'box'}, figsize=(4, 3))
# axes.hist([delays_window_1 - delays_window_2], bins, label=['Difference window delay'], normed=True)
