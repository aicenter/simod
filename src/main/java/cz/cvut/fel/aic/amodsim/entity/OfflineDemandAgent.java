...
public class OfflineDemandAgent extends Agent implements EventHandler, TransportableEntity {
	
    private static final org.slf4j.Logger LOGGER = LoggerFactory.getLogger(OfflineDemandAgent.class);
...
//	private long currentServiceDuration;
    ...
    public TimeTrip<SimulationNode> getTrip() {
        return trip;
    }
	...
	@Inject
	public OfflineDemandAgent(StationsDispatcher onDemandVehicleStationsCentral, EventProcessor eventProcessor, 
...
//				onDemandVehicle.getDemandTrip(this), this);
	...
	public void tripStarted(OnDemandVehicle vehicle) {
		if(state == DemandAgentState.DRIVING){
			try {
				...
				Logger.getLogger(OfflineDemandAgent.class.getName()).log(Level.SEVERE, null, ex);
			}
		}
		else{
	...
	public interface DemandAgentFactory {
		public OfflineDemandAgent create(String agentId, int id, TimeTrip<SimulationNode> osmNodeTrip);
	}
