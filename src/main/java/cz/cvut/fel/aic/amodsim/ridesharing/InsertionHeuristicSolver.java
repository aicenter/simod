...
public class InsertionHeuristicSolver extends DARPSolver {
    ...
    @Inject public InsertionHeuristicSolver(TravelTimeProvider travelTimeProvider, TravelCostProvider travelCostProvider, OnDemandVehicleStorage vehicleStorage,
            PositionUtil positionUtil, AmodsimConfig config, TimeProvider timeProvider) {
    ...
    @Override public Map<RideSharingOnDemandVehicle, DriverPlan> solve(List<OnDemandRequest> requests) {
        callCount++;
        ...
        if (bestPlan != null) {
            ...
            for (DriverPlanTask task : bestPlan) {
                if (previousTask != null) {
                    currentDelay += travelTimeProvider.getTravelTime(servingVehicle, previousTask.getLocation(), task.getLocation());
                }
        ...
        if (callCount % InsertionHeuristicSolver.INFO_PERIOD == 0) {
            ...
            sb.append("Simulation time: ").append(timeProvider.getCurrentSimTime()).append(readableTime(timeProvider.getCurrentSimTime() * 1000000)).append("\n");
    ...
    private boolean canServeRequest(RideSharingOnDemandVehicle vehicle, OnDemandRequest request) {
        ...
        boolean canServe = travelTimeProvider.getTravelTime(vehicle, vehicle.getPosition(), request.getPosition()) < maxDelayTime;

        return canServe;
    }

    private PlanData getOptimalPlan(RideSharingOnDemandVehicle vehicle, OnDemandRequest request) {
        long minTimeFromProvider = (long) travelTimeProvider.getTravelTime(vehicle, request.getDemandAgent().getPosition(), request.getTargetLocation());
        ...
        for (int pickupOptionIndex = 1; pickupOptionIndex <= currentPlan.getLength(); pickupOptionIndex++) {
            ...
            for (int dropoffOptionIndex = pickupOptionIndex + 1; dropoffOptionIndex <= currentPlan.getLength() + 1; dropoffOptionIndex++) {
                DriverPlan potentialPlan = insertIntoPlan(currentPlan, pickupOptionIndex, dropoffOptionIndex, request, vehicle, minTimeFromProvider);
    ...
    private DriverPlan insertIntoPlan(DriverPlan currentPlan, int pickupOptionIndex, int dropoffOptionIndex, OnDemandRequest request, RideSharingOnDemandVehicle vehicle,
            long minTimeFromProvider) {
        ...
        for (int newPlanIndex = 0; newPlanIndex <= currentPlan.getLength() + 1; newPlanIndex++) {
            ...
            if (newPlanIndex == pickupOptionIndex) {
                newTask = new DriverPlanTask(DriverPlanTaskType.PICKUP, request.getDemandAgent(), request.getDemandAgent().getPosition());
            } else if (newPlanIndex == dropoffOptionIndex) {
                newTask = new DriverPlanTask(DriverPlanTaskType.DROPOFF, request.getDemandAgent(), request.getTargetLocation());
            ...
            if (previousTask != null) {
                planTravelTime += travelTimeProvider.getTravelTime(vehicle, previousTask.getLocation(), newTask.getLocation());
            }
    ...
    private void computePlanCost(RideSharingOnDemandVehicle vehicle, DriverPlan currentPlan, DriverPlan potentialPlan, OnDemandRequest request) {
        ...
        while (planIterator.hasNext()) {
            ...
            if (task.getDemandAgent() == request.getDemandAgent()) {
                ...
                if (planIterator.hasNext()) {
                    ...
                    if (task.getTaskType() == DriverPlanTaskType.PICKUP && nextTask.getDemandAgent() == request.getDemandAgent()) {
                        if (planIterator.hasNext()) {
                            DriverPlanTask nextNextTask = planIterator.next();
                            costAddition += travelTimeProvider.getTravelTime(vehicle, nextTask.getLocation(), nextNextTask.getLocation());
                            costAddition -= travelTimeProvider.getTravelTime(vehicle, previousTask.getLocation(), nextNextTask.getLocation());
                            break;
                        }
                    } else {
                        costAddition -= travelTimeProvider.getTravelTime(vehicle, previousTask.getLocation(), nextTask.getLocation());
    ...
    private void debugFail(OnDemandRequest request) {
        ...
        for (OnDemandVehicle tVvehicle : vehicleStorage) {
            ...
            if (vehicle.hasFreeCapacity()) {
                ...
                double distance = positionUtil.getPosition(vehicle.getPosition()).distance(positionUtil.getPosition(request.getPosition()));
                ...
                if (distance < maxDistance) {
                    // real feasibility check
                    // TODO compute from interpolated position
                    double travelTime = travelTimeProvider.getTravelTime(vehicle, vehicle.getPosition(), request.getPosition());
        ...
        if (!freeVehicle) {
            ...
            System.out.println(
                    "Request " + request.getDemandAgent().getTripId() + ": Cannot serve request - Too big distance: " + bestCartesianDistance + "m (max distance: " + maxDistance
                            + ")");
        } else if (bestTravelTimne > maxDelayTime) {
            System.out.println("Request " + request.getDemandAgent().getTripId() + ": Cannot serve request - Too big traveltime to startLoaction: " + bestTravelTimne);
        } else {
            System.out.println("Request " + request.getDemandAgent().getTripId() + ": Cannot serve request - Some other problem - all nearby vehicle plans infeasible?");
        }
    }
