...
//		empty plan
	...
	public List<PlanLayerTrip> getPlanForRendering(){
		...
		for(PlanAction action: currentPlan){
			if(action.getPosition() != lastPosition){
				VehicleTrip newTrip = tripsUtil.createTrip(lastPosition.id, action.getPosition().id, vehicle);
				trips.add(new PlanLayerTrip(newTrip.getLocations(), action));
			}
