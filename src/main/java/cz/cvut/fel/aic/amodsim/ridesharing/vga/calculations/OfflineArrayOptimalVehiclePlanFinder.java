 ...
 */
@Singleton
public class OfflineArrayOptimalVehiclePlanFinder<V extends IOptimalPlanVehicle> 
		extends OptimalVehiclePlanFinder<V>{
    
    private final int timeToStart;
    private static final org.slf4j.Logger LOGGER = LoggerFactory.getLogger(OfflineArrayOptimalVehiclePlanFinder.class);


	@Inject
	public OfflineArrayOptimalVehiclePlanFinder(StandardPlanCostProvider planCostComputation, AmodsimConfig config,
			...
			if(!goDeeper){
				...
				if(onboardRequestsOnly){
					try {
						...
						Logger.getLogger(OfflineArrayOptimalVehiclePlanFinder.class.getName()).log(Level.SEVERE, null, ex);
						return null;
					}
				}
