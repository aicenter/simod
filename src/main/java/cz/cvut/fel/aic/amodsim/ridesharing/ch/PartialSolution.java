...
public abstract class PartialSolution {
    ...
    public DijkstraSolution getDijkstraSolution(Map<Integer, CHNode> idsTochNodes, int index) {
        ...
        CHNode thisNode = idsTochNodes.get((int) getNodeId(currentIdx));
        ...
        while (currentIdx >= 0) {
            ...
            if (viaEdgeId != START_NODE_TO_START_NODE_PATH) {
                CHEdge viaEdge = findEdgeWithId(thisNode, viaEdgeId, from);
                edges.addFirst(viaEdge);
                CHNode nextNode = (from ? viaEdge.to : viaEdge.from);
    ...
    private boolean searchInFromDirection(CHNode node, long edgeId) {
    ...
    private CHEdge findEdgeWithId(CHNode node, long edgeId, boolean from) {
        for (CHEdge edge : (from ? node.outEdges : node.inEdges)) {
            if (edge.id == edgeId)
                return edge;
        }
    ...
    private static ByteBuffer makeCompactFormat(List<DijkstraSolution> individualNodeSolutions) {
        ...
        for (int i=0 ; i<individualNodeSolutions.size() ; i++) {
            ...
            CHNode n = ds.getLastNode();
            bb.putLong(nodeIdOffset+8*i, n.node.id);
            bb.putInt(contractionOrderOffset+4*i, n.contractionOrder);
            bb.putInt(totalDriveTimeOffset+4*i, ds.totalCost);
            
            List<CHEdge> directedEdges = ds.getDeltaEdges();

            if (directedEdges.size() == 1) {
                CHEdge de = directedEdges.get(0);
                bb.putLong(viaEdgesOffset+8*i, de.id);
